import { sortConfig } from './repair_alert_list.js';
...
export const initTableSorting = () => {
    ...
    const sortableHeaders = document.querySelectorAll('th [id^="list-column-repair-alert-"][id$="-sort-icon"]');
    ...
    sortableHeaders.forEach(header => {
        ...
        newHeader.addEventListener('click', () => {
            ...
            const iconId = newHeader.querySelector('[id^="list-column-repair-alert-"]').id;
            const match = iconId.match(/list-column-repair-alert-(.+)-sort-icon/);
    ...
    function updateSortIcons() {
        ...
        document.querySelectorAll('[id^="list-column-repair-alert-"][id$="-sort-icon"]').forEach(icon => {
            icon.className = 'fas fa-sort ml-1 text-gray-400';
        });

        // Actualizar ícono de la columna seleccionada si hay alguna
        if (sortConfig.column) {
            const selectedIcon = document.getElementById(`list-column-repair-alert-${sortConfig.column}-sort-icon`);
            if (selectedIcon) {
