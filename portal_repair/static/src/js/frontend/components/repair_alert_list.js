 ...
 */
export const reloadRepairAlertListPage = async () => {
    // Obtener el contenedor principal de la lista
    const pageListItems = document.getElementById('repair-alert-page-list-items');
    if(!pageListItems) return;

    // Obtener y validar el input de página actual
    const currentPageInput = document.getElementById('page-repair-alert-list-pagination-page');
    if(!currentPageInput) return;
    const currentPage = parseInt(currentPageInput.value);

    // Obtener y validar el campo de búsqueda
    const searchInput = document.getElementById('page-repair-alert-list-quick-search');
    if(!searchInput) return;
    const search = searchInput.value;

    // Obtener dominio de búsqueda avanzada si existe
    const domainInput = document.getElementById('page-repair-alert-list-advanced-search-domain');
    ...
    const paginationContainer = document.getElementById('page-repair-alert-list-pagination-container');
    ...
    const paginationPrevious = document.getElementById('repair-alert-list-pagination-previous');
    const paginationButton = document.querySelectorAll('.repair-alert-list-pagination-button');
    const paginationNext = document.getElementById('repair-alert-list-pagination-next');
 ...
 *
 */
export const initRepairAlertManagementListPage = () => {
    ...
    if(createButton) createButton.addEventListener('click', () => {
        // Open the modal
        const modal = document.getElementById('page-repair-alert-list-create-modal');
        if (modal) modal.classList.remove('hidden');
    });

    // Configurar botón para importar archivos
    const fileUploadButton = document.getElementById('page-list-repair-alert-tools-action-import');
    ...
    const batchDeleteButton = document.getElementById('page-list-repair-alert-batch-action-delete');
    if(batchDeleteButton) batchDeleteButton.addEventListener('click', async () => {
        ...
        if(confirm('Are you sure you want to delete these items?')) {
            const res = await rpc('/account/repair/batch/delete', {
                ids: selectedIds,
            });
