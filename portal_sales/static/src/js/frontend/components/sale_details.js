...
const reloadSaleDetailsChatter = async () => {
    const chatter = document.getElementById('sale_details-details-chat-messages');
    if (!chatter) return;

    const orderId = parseInt(chatter.dataset.orderId);
    const partnerId = parseInt(chatter.dataset.partnerId);

    const response = await rpc('/account/sales/chatter/fetch', {
        sale_id: orderId
    })
...
const initMessageSending = () => {
    ...
    const fileInput = document.getElementById('page-details-sale-message-attachment-input');
    const tokenInput = document.getElementById('page-details-sale-message-attachment-token');
    ...
    const sendMessage = async () => {
        ...
        try {
            const result = await fetch('/account/sales/chatter/post', {
                method: 'POST',
                body: formData
            });
            ...
            await reloadSaleDetailsChatter();
        } catch (error) {
...
document.addEventListener('DOMContentLoaded', () => {
    reloadSaleDetailsChatter();
    initMessageSending();

    document.addEventListener('portal_sales.sale_details_reload_request', (event) => {
        console.log('Reloading sale details chatter', event.detail);
        reloadSaleDetailsChatter();
    });
});
