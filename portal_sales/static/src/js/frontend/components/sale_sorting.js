import { sortConfig } from './sale_list.js';
...
export const initTableSorting = () => {
    ...
    const sortableHeaders = document.querySelectorAll('th [id^="list-column-sale-"][id$="-sort-icon"]');
    ...
    sortableHeaders.forEach(header => {
        ...
        newHeader.addEventListener('click', () => {
            ...
            const iconId = newHeader.querySelector('[id^="list-column-sale-"]').id;
            const match = iconId.match(/list-column-sale-(.+)-sort-icon/);
    ...
    function updateSortIcons() {
        ...
        document.querySelectorAll('[id^="list-column-sale-"][id$="-sort-icon"]').forEach(icon => {
            icon.className = 'fas fa-sort ml-1 text-gray-400';
        });

        // Actualizar ícono de la columna seleccionada si hay alguna
        if (sortConfig.column) {
            const selectedIcon = document.getElementById(`list-column-sale-${sortConfig.column}-sort-icon`);
            if (selectedIcon) {
