...
const goToStep = (step) => {
    ...
    // Show current step
    const stepEl = getElement(`sale-create-step-${step}`);
    ...
    const backBtn = getElement('sale-create-back-btn');
    const nextBtn = getElement('sale-create-next-btn');
    const submitBtn = getElement('sale-create-submit-btn');
    const newBtn = getElement('sale-create-new-btn');
    const exitBtn = getElement('sale-create-exit-btn');
    ...
    if (step === 2) {
        ...
        const form = getElement('page-sale-list-create-form');
        ...
        const senderCountryDisplay = getElement('sale-sender-country-name-display');
        const receiverCountryDisplay = getElement('sale-receiver-country-name-display');
...
const getPackages = () => {
    try {
        return JSON.parse(getElement('sale-orders-data-input')?.value || '[]');
    } catch (e) { return []; }
};

const savePackages = (orders, shouldRender = true) => {
    const input = getElement('sale-orders-data-input');
    if (input) input.value = JSON.stringify(orders);
    if (shouldRender) renderPackagesList();
    updateStep1Rates();
};

const renderPackagesList = () => {
    const container = getElement('sale-orders-list-container');
    const orders = getPackages();
    if (!container) return;

    if (orders.length === 0) {
        // Add one default if empty
        orders.push({ weight: 1.0, length: 10, width: 10, height: 10, units: 1 });
        savePackages(orders);
        return;
    }

    container.innerHTML = orders.map((pkg, index) => `
        <div class="order-item bg-white border ${index === 0 ? 'border-theme-light' : 'border-gray-200'} rounded-xl p-4 shadow-sm relative overflow-hidden group animate-fade-in-up">
            ...
            `<button type="button" class="remove-order-btn text-red-400 hover:text-red-600 p-1 transition-colors" data-index="${index}">
...
const getSelectedProducts = () => {
    const productsInput = getElement('page-sale-list-create-form-products-list');
...
const renderSelectedProducts = () => {
    ...
    const container = getElement('page-sale-list-create-form-products-line-items-container');
    if (!container) return;
    ...
    container.querySelectorAll('.product-qty-input').forEach(input => {
        input.addEventListener('change', (e) => {
            ...
            getElement('page-sale-list-create-form-products-list').value = JSON.stringify(currentProducts);
        });
    });
    ...
    container.querySelectorAll('.qty-plus').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            getElement('page-sale-list-create-form-products-list').value = JSON.stringify(prods);
...
const validateStep = async (step) => {
    ...
    if (step === 1) {
        ...
        const carrierId = getElement('sale-selected-carrier-id')?.value;
        ...
        if (pkgs.length === 0 || pkgs.some(p => p.weight <= 0)) {
            systemShowNotification('Please ensure all orders have a valid weight.', { type: 'error' });
            return false;
        }
 ...
 */
const renderCarriersList = (carriers, containerId = 'sale-carriers-list') => {
    ...
    const currentCarrierId = getElement('sale-selected-carrier-id')?.value;
...
const selectCarrier = (id) => {
    ...
    if (!id) {
        const input = getElement('sale-selected-carrier-id');
...
/**
 * Submits the sale creation form to the backend
 */
const submitSaleCreate = async () => {
    const form = getElement('page-sale-list-create-form');
    if (!form) return;
    ...
    try {
        ...
        const response = await rpc('/account/sale/create', {
            ...
            sender_id: getElement('sale-sender-address-id')?.value || false,
            ...
            carrier_id: getElement('sale-selected-carrier-id')?.value || false,
            orders_data: getElement('sale-orders-data-input')?.value,
            type: getElement('sale-type-input')?.value || 'return',
        ...
        if (response?.status === 'success') {
            ...
            if (typeof reloadSaleListPage === 'function') {
                reloadSaleListPage();
            }
        } else {
            if (typeof systemShowNotification === 'function') {
                systemShowNotification(response?.message || 'Error creating sale.', { type: 'error' });
            }
        }
    } catch (e) {
...
const renderSummary = (summary) => {
    const container = getElement('sale-summary-content');
    if (!container || !summary) return;

    // Hide Sender Selection Badge
    const selectedBadge = getElement('sale-sender-selected-badge');
    const searchInput = getElement('sale-sender-search');
    const hiddenSenderId = getElement('sale-sender-address-id');
            ...
            <div class="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-100">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-[10px] text-gray-400 uppercase">${summary.num_orders} Packages</p>
                    </div>
                </div>
                    ...
                    ${summary.labels.map(label => `
                        <a href="${label.label_url}" target="_blank" class="flex items-center justify-between p-3 bg-white border border-gray-200 rounded-lg hover:border-theme hover:bg-theme-light/10 transition-all group">
                            <div class="flex items-center gap-2">
                                ...
                                <div>
                                    <p class="text-[10px] font-black text-gray-800">${label.order_name}</p>
 ...
 */
const initSaleCreateModal = () => {
    const form = getElement('page-sale-list-create-form');

    // Submit
    const submitBtn = getElement('sale-create-submit-btn');
    if (submitBtn) submitBtn.addEventListener('click', submitSaleCreate);
    ...
    if (newBtn) {
        newBtn.addEventListener('click', () => {
            ...
            renderSelectedProducts();
            const ordersInput = getElement('sale-orders-data-input');
    ...
    // Add Package
    const addPkgBtn = getElement('sale-add-order-btn');
    if (addPkgBtn) {
    ...
    document.addEventListener('modalClosed', (e) => {
        if (e.detail.modalId !== 'page-sale-list-create-modal') return;
        const form = getElement('page-sale-list-create-form');
        if (form) form.reset();
    ...
    document.addEventListener('modalOpened', (e) => {
        if (e.detail.modalId === 'page-sale-list-create-modal' || e.detail.modalId === 'sale-create-modal') {
            goToStep(1);
...
const updateStep1Rates = async () => {
    const modal = getElement('page-sale-list-create-modal');
    ...
    const container = getElement('sale-step1-carriers-section');
    const list = getElement('sale-step1-carriers-list');

    if (!senderCountryId || !receiverCountryId || pkgs.length === 0) {
        console.log('Skipping rates: missing countries or orders', { senderCountryId, receiverCountryId, pkgCount: pkgs.length });
    ...
    rateTimeout = setTimeout(async () => {
        console.log('Executing RPC /account/sale/calculate_rates', { senderCountryId, receiverCountryId, pkgs });
        try {
            const response = await rpc('/account/sale/calculate_rates', {
                orders_data: JSON.stringify(pkgs),
            ...
            if (response.status === 'success') {
                renderCarriersList(response.carriers, 'sale-step1-carriers-list');
                const currentCarrierId = getElement('sale-selected-carrier-id')?.value;
...
const initSenderAddressSearch = () => {
    const searchInput = getElement('sale-sender-search');
    const resultsContainer = getElement('sale-sender-search-results');
    const hiddenId = getElement('sale-sender-address-id');
    const selectedBadge = getElement('sale-sender-selected-badge');
    const selectedName = getElement('sale-sender-selected-name');
    const clearBtn = getElement('sale-sender-clear-btn');
    const form = getElement('page-sale-list-create-form');
    ...
    searchInput.addEventListener('input', () => {
        ...
        senderSearchTimeout = setTimeout(async () => {
            try {
                const res = await rpc('/account/sale/address/search', { query, address_type: 'sender' });
...
const initZipSearch = (type) => {
    const searchInput = getElement(`sale-${type}-zip-search`);
    const resultsContainer = getElement(`sale-${type}-zip-results`);
    const form = getElement('page-sale-list-create-form');
    ...
    searchInput.addEventListener('input', () => {
        ...
        zipSearchTimeout = setTimeout(async () => {
            try {
                const res = await rpc('/account/sale/zip/search', { query, country_id: countryId });
...
const renderStockKanban = (quants) => {
    const container = getElement('sale-stock-kanban-container');
    if (!container) return;
    ...
    container.querySelectorAll('.stock-item-card').forEach(card => {
        card.addEventListener('click', () => {
            ...
            if (window.Modal) window.Modal.close('sale-stock-selector-modal');
        });
    });
};
...
const addMappingToLines = (id, name, sku) => {
    ...
    getElement('page-sale-list-create-form-products-list').value = JSON.stringify(products);
    renderSelectedProducts();
};

const initStockSelector = () => {
    const btn = getElement('page-sale-list-create-form-products-add-stock-btn');
    if (!btn) return;

    btn.addEventListener('click', async () => {
        if (window.Modal) window.Modal.open('sale-stock-selector-modal');
        try {
            const res = await rpc('/account/sale/unmapped_products', {});
...
const initRMAProductSelector = () => {
    const btn = getElement('page-sale-list-create-form-products-add-rma-product-btn');
    if (!btn) return;

    btn.addEventListener('click', async () => {
        if (window.Modal) window.Modal.open('sale-product-map-selector-modal');
        try {
            const res = await rpc('/account/sale/product_maps', {});
            if (res.status === 'success') {
                const container = getElement('sale-product-map-kanban-container');
                if (!container) return;
                ...
                container.querySelectorAll('.rma-item-card').forEach(card => {
                    card.addEventListener('click', () => {
                        addMappingToLines(card.dataset.id, card.dataset.name, card.dataset.sku);
                        if (window.Modal) window.Modal.close('sale-product-map-selector-modal');
                    });
                });
            }
        } catch (e) { console.error(e); }
