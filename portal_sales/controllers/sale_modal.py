...
from odoo.addons.portal_sales.controllers.sale_details import PortalSaleDetailsController


class PortalSaleModalController(PortalSaleDetailsController):
    """Controller for sale creation modal and search endpoints"""

    @http.route('/account/sale/calculate_rates', type='json', auth='user')
    def account_sale_calculate_rates(self, **post):
        """Calculate shipping rates for various carriers based on order data and destination"""
        self._ensure_user_lang_context()
        orders_data = json.loads(post.get('orders_data') or '[]')
        ...
        _logger.info("Computing rates for %d orders. Route: %s -> %s", len(orders_data), sender_country_id, receiver_country_id)
        ...
        for p in orders_data:
            ...
            total_quantity += float(p.get('units', 1.0)) # Each unit in order adds to total qty for rules
    ...
    @http.route('/account/sale/create', type='json', auth='user')
    def account_sale_create(self, **post):
        """Create a new sale order"""
        self._ensure_user_lang_context()
        partner = request.env.user.partner_id

        StockQuantPackage = request.env['sale.order'].sudo()
        ...
        orders_data = json.loads(post.get('orders_data') or '[]')
        sale_type = post.get('type', 'return')

        if not orders_data:
            return {'status': 'error', 'message': _('Please add at least one order')}

        if not carrier_id:
            return {'status': 'error', 'message': _('Please select a carrier')}

        total_weight = float(sum([float(p.get('weight', 0)) for p in orders_data]))
        number_of_orders = len(orders_data) # Each UI row represents one order-item (bulto)
        total_units = sum([float(p.get('units', 1)) for p in orders_data])
        ...
        order_vals = {
            ...
            'number_of_orders': number_of_orders or 1,
            'type': sale_type,
            'notes': notes,
        ...
        if hasattr(StockQuantPackage, 'set_name_based_on_account'):
            order_vals['name'] = StockQuantPackage.set_name_based_on_account(account_partner)

        # Set dimensions from the first order if available (as declared by client)
        if orders_data:
            p0 = orders_data[0]
            if 'length' in p0: order_vals['packaging_length'] = float(p0.get('length', 0))
            if 'width' in p0: order_vals['width'] = float(p0.get('width', 0))
            if 'height' in p0: order_vals['height'] = float(p0.get('height', 0))

        try:
            # 1. CREATE PACKAGES AND RMA LINES
            created_orders = []
            GenericProduct = request.env.ref('portal_sales.product_generic_sale', raise_if_not_found=False)

            for i, p in enumerate(orders_data):
                # Specific values for this individual order
                ...
                order = StockQuantPackage.create(p_vals)
                created_orders.append(order)

                # Create RMA Line for this specific order
                ...
                if mapping:
                    request.env['rma.order.line'].sudo().create({
                        'order_id': order.id,
            ...
            if carrier:
                if carrier.delivery_type == 'fixed':
                    ...
                    total_volume = sum([(float(p.get('length', 0)) * float(p.get('width', 0)) * float(p.get('height', 0))) / 1000000.0 for p in orders_data])
                    total_price = carrier._get_price_from_picking(total=0.0, weight=total_weight, volume=total_volume, quantity=float(number_of_orders))
                else:
                    total_price = float(getattr(carrier, 'fixed_price', 10.0)) + (total_weight * 1.2)

            # Create a nice origin string safely
            origin_list = []
            for i, p in enumerate(created_orders):
                if i < 3:
            ...
            if sale_order_id:
                ...
                request.env['sale.order.line'].sudo().create({
                    ...
                    'name': _("Shipping Service (%d orders, %d total units, %.2f kg)") % (number_of_orders, total_units, total_weight),
                    'product_uom_qty': 1.0,
                ...
                for order in created_orders:
                    request.env['sale.order.line'].sudo().create({
                        'order_id': sale_order_id,
                        'product_id': shipping_product_id,
                        'name': _("Bundle / Package: %s") % order.name,
                        'product_uom_qty': 1.0,
                        'price_unit': 0.0,
                    })
            ...
            for i, order in enumerate(created_orders):
                labels.append({
                    ...
                    'tracking_ref': f"TRK-{order.id}-{i+1}",
                    'label_url': f"/account/sale/label_dummy/{order.id}/{i+1}"
                })
            ...
            if hasattr(user, 'send_portal_user_recent_activity'):
                ...
                user.send_portal_user_recent_activity(
                    "Sales created: %s",
                    "New %d orders and sale order",
                    "fas fa-box",
                    "success",
                    message_args=[display_names, len(created_orders)]
                )

            return {
                'status': 'success', 
                'message': _('Sale created successfully'),
                'summary': {
                    'orders': [{'id': p.id, 'name': p.name} for p in created_orders],
                    'order_name': sale_order.name if sale_order else _('New Order'),
                    'num_orders': int(number_of_orders),
                    'total_cost': float(total_price),
                    'cost_per_order': float(total_price / number_of_orders) if number_of_orders > 0 else 0.0,
            ...
            _logger.error("Error creating sale: %s", str(e))
            return {'status': 'error', 'message': str(e)}

    @http.route('/account/sale/label_dummy/<int:order_id>/<int:order_num>', type='http', auth='user')
    def account_sale_label_dummy(self, order_id, order_num, **kw):
        """Return a dummy empty PDF label"""
        # Very simple PDF content
        pdf_content = b"%PDF-1.4\n1 0 obj\n<< /Type /Catalog /Pages 2 0 R >>\nendobj\n2 0 obj\n<< /Type /Pages /Kids [3 0 R] /Count 1 >>\nendobj\n3 0 obj\n<< /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] >>\nendobj\nxref\n0 4\n0000000000 65535 f\n0000000009 00000 n\n0000000058 00000 n\n0000000115 00000 n\ntrailer\n<< /Size 4 /Root 1 0 R >>\nstartxref\n0\n%%EOF"
        return request.make_response(pdf_content, headers=[
            ('Content-Type', 'application/pdf'),
            ('Content-Disposition', f'attachment; filename="label_{order_id}_{order_num}.pdf"')
        ])

    @http.route('/account/sale/address/search', type='json', auth='user')
    def account_sale_address_search(self, query='', address_type='sender', **kw):
    ...
    @http.route('/account/sale/address/create', type='json', auth='user')
    def account_sale_partner_create(self, **post):
    ...
    @http.route('/account/sale/product-catalog', type='json', auth='user')
    def account_sale_product_catalog(self, page=1, search='', **post):
    ...
    @http.route('/account/sale/product-search', type='json', auth='user')
    def account_sale_product_search(self, term='', **kw):
    ...
    @http.route('/account/sale/carrier-search', type='json', auth='user')
    def account_sale_carrier_search(self, term='', **kw):
    ...
    @http.route('/account/sale/zip/search', type='json', auth='user')
    def account_sale_zip_search(self, query='', country_id=False, **kw):
    ...
    @http.route('/account/sale/unmapped_products', type='json', auth='user')
    def account_sale_unmapped_products(self, search='', **kw):
        ...
        for p in products:
            result.append({
                ...
                'name': p.name,
                'code': p.default_code,
                'image_url': f'/account/sale/product_image/{p.id}'
            })

        return {'status': 'success', 'quants': result}

    @http.route('/account/sale/product_image/<int:product_id>', type='http', auth='user')
    def account_sale_product_image(self, product_id, **kw):
    ...
    @http.route('/account/sale/product/map/quick_create', type='json', auth='user')
    def account_sale_product_map_quick_create(self, **post):
    ...
    @http.route('/account/sale/address/get_details', type='json', auth='user')
    def account_sale_address_get_details(self, partner_id, **kw):
