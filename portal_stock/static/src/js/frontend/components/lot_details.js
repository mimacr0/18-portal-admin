...
const reloadLotDetailsChatter = async () => {
    const chatter = document.getElementById('lot_details-details-chat-messages');
    if (!chatter) return;

    const lotId = parseInt(chatter.dataset.lotId);
    const partnerId = parseInt(chatter.dataset.partnerId);

    const response = await rpc('/portal_stock/lot/details/chatter/fetch', {
        lot_id: lotId
    })

    const messages = response?.data['mail.message'] || [];
    if (messages.length === 0) {
        chatter.innerHTML = '<p class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm">No se encontraron mensajes.</p>';
        return;
    }
...
const initMessageSending = () => {
    ...
    const sendButton = document.getElementById('page-details-lot_details-send-message');
    const messageInputContainer = document.getElementById('message-input-container');
    const attachButton = document.querySelector('.message-attach-button');
    const fileInput = document.getElementById('page-details-lot_details-message-attachment-input');
    const tokenInput = document.getElementById('page-details-lot_details-message-attachment-token');
    ...
    const sendMessage = async () => {
        ...
        try {
            const result = await fetch(`/portal_stock/lot/details/chatter/post?lot_id=${lotId}`, {
                method: 'POST',
                body: formData
            });
...
document.addEventListener('DOMContentLoaded', () => {
    reloadLotDetailsChatter();
    initMessageSending();

    document.addEventListener('portal_stock.lot_details_reload_request', (event) => {
        console.log('Reloading lot details chatter', event.detail);
        reloadLotDetailsChatter();
    });
});
