import { sortConfig } from './reception_list.js';
...
export const initTableSorting = () => {
    ...
    const sortableHeaders = document.querySelectorAll('th [id^="list-column-reception-"][id$="-sort-icon"]');
    ...
    sortableHeaders.forEach(header => {
        ...
        newHeader.addEventListener('click', () => {
            ...
            const iconId = newHeader.querySelector('[id^="list-column-reception-"]').id;
            const match = iconId.match(/list-column-reception-(.+)-sort-icon/);
    ...
    function updateSortIcons() {
        ...
        document.querySelectorAll('[id^="list-column-reception-"][id$="-sort-icon"]').forEach(icon => {
            icon.className = 'fas fa-sort ml-1 text-gray-400';
        });

        // Actualizar ícono de la columna seleccionada si hay alguna
        if (sortConfig.column) {
            const selectedIcon = document.getElementById(`list-column-reception-${sortConfig.column}-sort-icon`);
            if (selectedIcon) {
