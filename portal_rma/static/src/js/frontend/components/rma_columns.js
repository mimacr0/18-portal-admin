/**
 * RMA Column Selector Component
 * Handles dynamic visibility of table columns with localStorage persistence.
 */

const STORAGE_KEY = 'portal_rma_visible_columns';
 ...
 * @param {Object} visibility 
 */
export const applyColumnVisibility = (visibility = null) => {
    ...
    Object.keys(settings).forEach(colId => {
        ...
        const checkbox = document.querySelector(`.rma-column-toggle[data-column-id="${colId}"]`);
        if (checkbox) {
...
/**
 * Initializes the column selector component.
 */
export const initRmaColumnSelector = () => {
    const btn = document.getElementById('rma-columns-selector-btn');
    const dropdown = document.getElementById('rma-columns-dropdown');
    const container = document.getElementById('rma-columns-selector-container');
    ...
    const toggles = document.querySelectorAll('.rma-column-toggle');
    toggles.forEach(toggle => {
        toggle.addEventListener('change', (e) => {
            ...
            applyColumnVisibility(currentSettings);
        });
    });
